...
3. `ralph run <feature>` enters the main loop:
   ...
   - **Verify-at-top**: runs verification first — if story already passes, mark passed and skip to next (guarded by `state.IsAttempted()` to only pre-verify stories previously given to a provider)
...
```json
{
  ...
  "attempted": ["US-001", "US-002", "US-003", "US-005"]
}
```
...
- **Per-story attempted guard**: Verify-at-top uses `state.IsAttempted(story.ID)` instead of the branch-level `HasNonRalphChanges()`. A story is only pre-verified if it was previously given to a provider. This prevents false positives where unimplemented stories pass vacuously after another story's commit makes `HasNonRalphChanges()` return true. `HasNonRalphChanges()` is still available in git.go but no longer used by the verify-at-top gate.
...
- **Verify-at-top-of-loop**: Each loop iteration verifies the story BEFORE implementation. If the story already passes all checks, it's marked passed and skipped — no provider spawn needed. This replaces the old pre-verify phase and enables the "infinite loop" pattern: modify PRD → run → verify-at-top catches already-implemented work. **Per-story attempted guard**: verify-at-top only runs for stories where `state.IsAttempted(story.ID)` is true (the story was previously given to a provider). This prevents false positives where unimplemented stories pass vacuously because the test suite doesn't have story-specific tests yet. Stories are marked attempted via `state.MarkAttempted(story.ID)` before provider spawn, ensuring crash recovery works correctly.
...
- **Story state helpers**: `RunState` methods: `MarkPassed(id)` appends to passed[], `MarkSkipped(id, reason)` appends to skipped[] + sets lastFailure, `MarkFailed(id, reason, maxRetries)` increments retries and auto-skips at threshold, `UnmarkPassed(id)` removes from passed (for regression detection), `MarkAttempted(id)` records a story was given to a provider (idempotent), `IsAttempted(id)` checks if a story was previously attempted. `GetNextStory(def, state)` returns first non-passed, non-skipped story by priority. `AllComplete(def, state)` returns true when every story is in passed or skipped.
...
- **`promptYesNo`/`promptChoice` are EOF-safe**: On EOF or read error (e.g., closed stdin in non-interactive contexts), `promptYesNo` returns `false` (declines) and `promptChoice` returns `""` (no match, callers fall through to nil return). Prevents infinite loops when `ralph verify` is run without a terminal.
...
- `schema_test.go` - PRD definition validation, RunState methods (MarkPassed/MarkFailed/MarkSkipped/UnmarkPassed/MarkAttempted/IsAttempted), GetNextStory, AllComplete, GetPendingStories, learning deduplication, LoadPRDDefinition, ValidatePRDDefinition, LoadRunState, SaveRunState, v2 error, IsUIStory, attempted backwards compat
...
- `resources_test.go` - ResourceManager, ResourcesConfig, cache operations, ensureResourceSync, GetCachedResources, CachedResource version field, versioned repo sync skip (empty Branch)
...
- `external_git_test.go` - ExternalGitOps (ref field), Exists, GetRepoSize, DetectDefaultBranch fallback
