...
func TestMergePRDState_Basic(t *testing.T) {
	oldPRD := &PRD{
		Run: Run{
			Learnings: []string{"learning1"},
		},
		UserStories: []UserStory{
			{ID: "US-001", Passes: true, Retries: 1, LastResult: &LastResult{Commit: "abc123", Summary: "Done"}, Notes: "worked"},
			{ID: "US-002", Passes: false, Retries: 2, Notes: "failed twice"},
		},
	}
	...
	newPRD := &PRD{
		Run: Run{},
		UserStories: []UserStory{
			{ID: "US-001", Title: "Updated title"},
			{ID: "US-002", Title: "Also updated"},
		},
	}

	MergePRDState(oldPRD, newPRD)

	s1 := GetStoryByID(newPRD, "US-001")
	if !s1.Passes {
		t.Error("expected US-001 passes=true after merge")
	}
	if s1.Retries != 1 {
		t.Errorf("expected US-001 retries=1, got %d", s1.Retries)
	}
	if s1.LastResult == nil || s1.LastResult.Commit != "abc123" {
		t.Error("expected US-001 lastResult preserved")
	}
	if s1.Notes != "worked" {
		t.Errorf("expected US-001 notes='worked', got '%s'", s1.Notes)
	}

	s2 := GetStoryByID(newPRD, "US-002")
	if s2.Retries != 2 {
		t.Errorf("expected US-002 retries=2, got %d", s2.Retries)
	}
	if s2.Notes != "failed twice" {
		t.Errorf("expected US-002 notes='failed twice', got '%s'", s2.Notes)
	}

	if len(newPRD.Run.Learnings) != 1 || newPRD.Run.Learnings[0] != "learning1" {
		t.Errorf("expected learnings preserved, got %v", newPRD.Run.Learnings)
	}
}

func TestMergePRDState_NilOldPRD(t *testing.T) {
	newPRD := &PRD{
		Run: Run{},
		UserStories: []UserStory{
			{ID: "US-001", Title: "New story"},
		},
	}

	MergePRDState(nil, newPRD)

	s := GetStoryByID(newPRD, "US-001")
	if s.Passes {
		t.Error("expected passes=false for nil old PRD")
	}
	if s.Retries != 0 {
		t.Errorf("expected retries=0, got %d", s.Retries)
	}
}

func TestMergePRDState_NewStoriesGetFreshState(t *testing.T) {
	oldPRD := &PRD{
		UserStories: []UserStory{
			{ID: "US-001", Passes: true},
		},
	...
	newPRD := &PRD{
		UserStories: []UserStory{
			{ID: "US-001", Title: "Old story"},
			{ID: "US-003", Title: "Brand new story"},
		},
	}

	MergePRDState(oldPRD, newPRD)

	s3 := GetStoryByID(newPRD, "US-003")
	if s3.Passes {
		t.Error("expected new story passes=false")
	}
	if s3.Retries != 0 {
		t.Errorf("expected new story retries=0, got %d", s3.Retries)
	}
	if s3.Blocked {
		t.Error("expected new story blocked=false")
	}
}

func TestMergePRDState_RemovedStoriesDropped(t *testing.T) {
	oldPRD := &PRD{
		UserStories: []UserStory{
			{ID: "US-001", Passes: true},
	...
	if GetStoryByID(newPRD, "US-002") != nil {
		t.Error("removed story should not be in new PRD")
	}
}

func TestMergePRDState_BlockedPreserved(t *testing.T) {
	oldPRD := &PRD{
		UserStories: []UserStory{
			{ID: "US-001", Blocked: true, Notes: "Depends on external API", Retries: 3},
		},
	}

	newPRD := &PRD{
		UserStories: []UserStory{
			{ID: "US-001", Title: "Updated description"},
		},
	}
	...
	s := GetStoryByID(newPRD, "US-001")
	if !s.Blocked {
		t.Error("expected blocked=true after merge")
	}
	if s.Notes != "Depends on external API" {
		t.Errorf("expected notes preserved, got '%s'", s.Notes)
	}
	if s.Retries != 3 {
		t.Errorf("expected retries=3, got %d", s.Retries)
	}
}

func TestMergePRDState_LearningsPreserved(t *testing.T) {
	ts := "2024-01-15T10:00:00Z"
	currentID := "US-002"
	oldPRD := &PRD{
		Run: Run{
	...
	if len(newPRD.Run.Learnings) != 2 {
		t.Errorf("expected 2 learnings, got %d", len(newPRD.Run.Learnings))
	}
	if newPRD.Run.StartedAt == nil || *newPRD.Run.StartedAt != ts {
		t.Error("expected startedAt preserved")
	}
	if newPRD.Run.CurrentStoryID == nil || *newPRD.Run.CurrentStoryID != currentID {
		t.Error("expected currentStoryID preserved")
	}
}

func TestMergePRDStateSummary(t *testing.T) {
	tests := []struct {
		name   string
		oldPRD *PRD
		newPRD *PRD
	...
	}{
		{
			"nil old PRD",
			nil,
			&PRD{UserStories: []UserStory{{ID: "US-001"}}},
			"",
		},
		{
			"matched with passed and blocked",
			...
			"Preserved state for 3 stories (1 passed, 1 blocked). 1 new stories added.",
		},
		{
			"only new stories",
			&PRD{UserStories: []UserStory{
		...
		},
		{
			...
			"Preserved state for 1 stories.",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := MergePRDStateSummary(tt.oldPRD, tt.newPRD)
			if got != tt.want {
